...
async def test_auth_full_flow(async_client: AsyncClient):
    ...
    res = await async_client.post("/api/v1/auth/signup", json=victim_signup_data)
    ...
    assert me_data["victim_id"] is not None
    ...
    c_res = await async_client.post("/api/v1/auth/signup", json=counselor_signup_data)
    assert c_res.status_code == 201, c_res.text
    c_token = c_res.json()["access_token"]

    # 5. Get /me for Counselor
    c_me_res = await async_client.get(
        "/api/v1/auth/me",
    ...
    assert c_me_data["counselor_id"] is not None
    ...
    a_res = await async_client.post("/api/v1/auth/signup", json=admin_signup_data)
    assert a_res.status_code == 201, a_res.text
    a_token = a_res.json()["access_token"]

    # 7. Get /me for Admin
    a_me_res = await async_client.get(
        "/api/v1/auth/me",
    ...
    assert a_me_data["admin_id"] is not None
